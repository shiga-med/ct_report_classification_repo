...
python loss_comparison_fixed.py \
  ...
  --val_csv ../data/preprocessed/val.csv \
  --test_csv ../data/preprocessed/test.csv \
  --losses ce focal cbloss ib ibfocal \
...
- `--repeat-eval N`: Repeat each evaluation N times with different demonstration samples
...
python loss_comparison_fixed.py \
  ...
  --losses ce focal ib --seeds 12 22 32 42 52
